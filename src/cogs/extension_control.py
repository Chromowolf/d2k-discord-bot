        ...
        try:
            ...
            if ext_path in self.bot.extensions:
                ...
                await interaction.response.send_message(
                    f"❌ The extension `{ext}` is already loaded.",
                    ephemeral=True
                )
                logger.warning(f"Attempted to load existing extension: {ext}")
                return
            ...
            try:
                ...
                # noinspection PyUnresolvedReferences
                await interaction.followup.send(f"✅ Successfully loaded `{ext}` extension.", ephemeral=True)
                logger.info(f"Loaded extension: {ext}")
            except ModuleNotFoundError:
                # noinspection PyUnresolvedReferences
                await interaction.followup.send(f"❌ Extension `{ext}` not found! Make sure it exists in `cogs/`.", ephemeral=True)
                logger.error(f"Failed to load xxtension `{ext}`: Module not found.")
            except commands.ExtensionFailed as e:
                # noinspection PyUnresolvedReferences
                await interaction.followup.send(f"❌ Extension `{ext}` exists but failed to load!\nError: {e}", ephemeral=True)
                logger.error(f"Failed to load xxtension `{ext}`: {e}")
            except Exception as e:
                # noinspection PyUnresolvedReferences
                await interaction.followup.send(f"❌ Unexpected error loading `{ext}`: {e}", ephemeral=True)
    ...
    async def unloadext(self, interaction: discord.Interaction, ext: str):
        ...
        try:
            ...
            try:
                ...
                await self.bot.sync_commands()  # Resync commands after modification
                # noinspection PyUnresolvedReferences
                await interaction.followup.send(f"✅ Successfully unloaded `{ext}` extension.", ephemeral=True)
